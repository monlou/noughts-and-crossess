...
type ITicTacToeGame<'Player> = 
    ...
    // the current state of the board represented by a function that maps squares with (row,column) coordinates 
    // to the piece on that square ("X" for Cross, "O" for Nought and "" for empty)
    abstract member getPiece: int * int -> string 
